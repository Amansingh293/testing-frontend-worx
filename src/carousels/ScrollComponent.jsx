...
const ScrollComponent = () => {
  ...
  return (
    <div className="relative w-full flex flex-col items-center space-y-10  ">
      ...
      <div className="realtive w-full p-6 " ref={containerRef}>
        {sections.map((section, index) => (
          <div className="sticky top-[60px] max-w-6xl h-[600px] bg-white flex justify-center pl-8">
            <div
              key={index}
              ref={(el) => (sectionRefs.current[index] = el)}
            ...
            >
              {/* <div className="absolute top-[0px] left-3 w-[20px] h-[150px] bg-white"></div> */}
              <div className="w-full flex justify-between p-6 m-10">
                {/* Left Section - Takes 1 column */}
                <div className="w-[300px] flex flex-col justify-center items-start">
                  <h3 className="stretch text-[#252B37] text-[20px] font-inter font-semibold leading-8">
                    {section.heading}
                  ...
                  <Link to="/login"><button class="relative mt-4 rounded-[36px] border border-[#207C97] bg-[#207C97] text-white text-sm font-medium leading-[20px] py-2 px-4 overflow-hidden group">
                    <span class="relative z-10 group-hover:text-[#207C97] transition-colors duration-300"> {section.button}</span>
                    <span class="absolute rounded-full inset-0 bg-white transform scale-0 group-hover:scale-100 transition-all duration-500 ease-out origin-bottom"></span>
                  </button></Link>
                </div>

                {/* Right Section (Video) - Takes 2 columns */}
                <div className="max-w-3xl h-[300px] flex justify-center border border-gray-300 rounded-md overflow-hidden ml-6 mt-8">
                  {/* <video
...
<div className="absolute top-[90px] md:left-[76px] lg:left-[74px] xl:custom-left transform -translate-y-1/2 flex items-center justify-center w-[52px] h-[52px] text-white rounded-full border-2 border-dashed border-gray-300 bg-white">
  <AnimatePresence mode="wait">
    {viewedSections.has(index) ? (
      <motion.div 
        key="tick"
