...
export default function BlogSlide({text, goto, classname}) {
  ...
  return (
    <div className={`flex flex-col items-${classname} text-center w-full p-6 mx-auto`}>
      <h2 className={`mb-[64px] text-[#252B37] text-left text-[30px] font-inter font-medium leading-[38px]`}>{text}</h2>
      <Carousel setApi={setApi} className="w-full">
        <CarouselContent>
          {blogs.map((blog, idx) => (
            <CarouselItem key={idx} className="sm:basis-2/5 basis-full flex justify-center ">
              <Card onClick={handleClick} className="mb-2 w-auto h-auto bg-white flex flex-col items-start cursor-pointer rounded-[10px] hover:shadow-lg transition duration-300 overflow-visible">
              <img
              src={blog.image}
              alt={blog.title}
            ...
            <p className="mt-2 stretch text-[#252B37] text-left font-inter text-[16px] font-semibold leading-[20px]">{blog.title}</p>
        ...
        <div className="flex justify-between gap-4 w-full mt-4">
          ...
          <div className="flex justify-center items-center">
            <button onClick={() => api?.scrollTo(current + 1)}  disabled={current === blogs.length-1}
              className={`px-4 py-2 transition-colors ${current === blogs.length-1 ? 'text-gray-400' : 'text-[#252B37] hover:text-[#207C97]'}`}
            >Next</button>
